"""Base training configuration and task config YAML loading.

``DefaultTrainingConfig`` originates from examples/experiments/config.py of the original
hil-serl repository (rail-berkeley/hil-serl); the YAML loading helpers are later additions.
...
consistent precedence: class defaults < task config YAML < CLI overrides.
"""
...
def load_task_config(path: str) -> dict[str, Any]:
    """Load a task config YAML and return top-level overrides (excluding ``adapter:``)."""
...
def build_train_config(
    ...
    task config YAML identically. Priority: class defaults < task config YAML
    < extra_overrides.

    Args:
        config_path: Experiment YAML path. When None or empty, the TrainConfig's
            own default ``task_config_path`` (if any) is used.
    ...
    from serl_framework.train.mappings import resolve_task_config_class

    config = resolve_task_config_class(config_mapping, exp_name)()
    if config_path:
        config.task_config_path = config_path
    overrides: dict[str, Any] = {}
    effective_path = str(getattr(config, "task_config_path", "") or "")
    if effective_path and os.path.isfile(effective_path):
        overrides = load_task_config(effective_path)
    if extra_overrides:
...
class DefaultTrainingConfig:
    ...
    # Optional path to a task config YAML applied on top of the class defaults
